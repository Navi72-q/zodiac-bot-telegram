...
if __name__ == "__main__":
   # Считывание конфигурационного файла или его создания с настройками по умолчанию
   ...
   if os.path.exists(current_path + '/' + 'config.ini'):
      ...
      config['DEFAULT'] = {'token': '',
         ...
         with open(current_path + '/' + 'config.ini', 'w') as configfile:
            ...
            # Создание директорий
            try:
                os.mekedirs(config['DEFAULT']['db_path'])
                os.mekedirs(config['DEFAULT']['log_dir_patch'])
            except OSError:
               pass
